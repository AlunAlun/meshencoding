var WebMesh = function(shaderPath, meshPath, options, callback){
    ...
    var splPath = splitPath(meshPath);
...
WebMesh.prototype.load = function(callback) {
    var xhr = new XMLHttpRequest();
    xhr.open('GET', this.filepath+this.filename);

    xhr.onload = function () {
        if (xhr.status >= 200 && xhr.status < 400){
            var jsonData = JSON.parse(xhr.responseText);
            this.parseJSON(jsonData, callback);
        }
    }.bind(this);
...
WebMesh.prototype.parseJSON = function(jsonData, callback) {
...
function ProgressiveWebMesh(shaderPath, file, options, callback) {
    ...
    this.load(shaderPath, file, options, callback)
}

var DRAWMODE = { PROGRESSIVE: 1, STATIC: 2};

ProgressiveWebMesh.prototype.load = function(shaderPath, file, options, callback) {
    this.file = file;
    var xhr = new XMLHttpRequest();
    xhr.open('GET', file);
    ...
    xhr.onload = function () {
        if (xhr.status >= 200 && xhr.status < 400){
            ...
            //not tested
            this.parseJSON(shaderPath, options, jsonData, callback);
...
ProgressiveWebMesh.prototype.parseJSON = function(shaderPath, options, jsonData, callback)  {
    ...
    for (var i = 0; i < this.levels.length; i++) {
        ...
        var theMesh = new WebMesh(shaderPath, this.file, options, callback);
        if (this.onLoadProgress)
            theMesh.onLoadProgress = this.onLoadProgress;
        this.meshes.push(theMesh);
    }

    this.meshes[0].parseJSON(this.levels[0], callback);
    this.ready = true;
...
ProgressiveWebMesh.prototype.update = function() {
    ...
    if (this.currentMesh < this.meshes.length-1) {
        if (this.meshes[this.currentMesh].ready) {
            ...
            this.currentMesh++;
            this.meshes[this.currentMesh].parseJSON(this.levels[this.currentMesh]);
        }
...
ProgressiveWebMesh.prototype.draw = function(model, view, proj, light_pos) {
    ...
    if (this.drawMode == DRAWMODE.PROGRESSIVE) {
        for (var i = this.meshes.length-1; i >= 0; i--) {
            if (this.meshes[i].ready){
                this.meshes[i].draw(model, view, proj, light_pos);
                break;
            }
        }
    }
    else
        this.meshes[this.selectedLevel].draw(model, view, proj, light_pos);
}
